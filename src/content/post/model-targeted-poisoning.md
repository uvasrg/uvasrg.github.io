...
tags = ["adversarial machine learning", "model-targeted poisoning attacks", "Fnu Suya", "Saeed Mahloujifar", "Anshuman Suri", "David Evans", "Yuan Tian", "ICML 2020"]
+++

## Data Poisoning Attacks

Machine learning models are often trained using data from untrusted sources, leaving them open to poisoning attacks where adversaries can use their control over a small fraction of that training data to poison the model in a particular way.
...
These poisoning attacks are typically one of two types:

- _Objective-driven_, where the adversary chooses poisoning points that maximize some target objective, or
- _Model-targeted_, where the adversary splits the attack into choosing a target model that satisfies the objective and then choosing poisoning points that induce the target model.

While objective-driven attacks are more commonly studied, the model-targeted approach has significant advantages. Objective-driven attacks must be designed for a specific objective, but model-targeted attacks can incorporate any attacker objective into the target model and can be applied to many different objectives.

## The Attack

Our attack requires the desired target model and the clean training data. We sequentially train a model on the mixture of the clean training points and the poisoning points found so far (which at the start is none) in order to generate an intermediate model. We then find a point that maximizes the loss difference between the intermediate model and the target model, and then add that point to the poisoning data for the next iteration.
...
The process repeats until some stopping condition is met (such as the maximum loss difference between the intermediate and target models being smaller than a threshold value). We prove two important features of our attack:
...
To test our attack, we use subpopulation and indiscriminate attack scenarios on SVM and linear regression models for the Adult, MNIST 1-7, and Dogfish datasets. We compare our attack to the state-of-the-art model targeted KKT attack [Koh, P. W. and Liang, P. Stronger data poisoning attacks break data sanitation defenses. <a href="https://arxiv.org/abs/1811.00741">arXiv preprint arXiv:1811.00741, 2018</a>].
...
Fnu Suya, Saeed Mahloujifar, Anshuman Suri, David Evans, Yuan Tian. <a href="https://arxiv.org/abs/2006.16469">Model-Targeted Poisoning Attacks with Provable Convergence.</a> In ICML 2020.
